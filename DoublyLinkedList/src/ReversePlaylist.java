...
/**
 * An instance of this class represents a ReversePlaylist object
 * 
 * @author Xuxiang Sun
 */
public class ReversePlaylist implements java.util.Iterator<Song> {
  public DoublyLinkedNode<Song> node;

  /**
   * Initializes a new ReversePlaylist with the specified node
   * 
   * @param node - expected to be the tail node of a doubly linked list
   */
  ...
  /**
   * Returns the current node and sets node to the previous node
   * 
  ...
  public Song next() {
    ...
    node = node.getPrevious(); // points node to previous node
